...
public final class BiomesPredicate implements BiPredicate<ServerLevel, BlockPos> {
  ...
  public static BiomesPredicate fromJson(@Nullable JsonElement json) {
    return json == null || json.isJsonNull() ? ANY : new BiomesPredicate(
      GsonHelper.getAsJsonArray(GsonHelper.convertToJsonObject(json, "biomes"), "biomes", new JsonArray())
        .asList()
        .stream()
        .map(biome -> ResourceKey.create(Registries.BIOME, new ResourceLocation(GsonHelper.convertToString(biome, "biome"))))
        .toList()
    );
  }
